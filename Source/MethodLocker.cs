...
namespace RimThreaded
{
    ...
    {
        public static Dictionary<object, ReaderWriterLockSlim> InstanceToLock =
            new Dictionary<object, ReaderWriterLockSlim>();

        public static Dictionary<Type, ReaderWriterLockSlim> DeclaringTypeToLock =
            new Dictionary<Type, ReaderWriterLockSlim>();

        public static Dictionary<MethodBase, ReaderWriterLockSlim> MethodBaseToLock =
            new Dictionary<MethodBase, ReaderWriterLockSlim>();
        ...
        public static void LockMethodOnInstance(Type Original, string Method, LockFlag Flag, Type[] OTypes = null)
        {
            if (Flag == LockFlag.ReaderLock)
                RimThreadedHarmony.Prefix(Original, typeof(MethodLocker), Method, destructive: false,
                    PatchMethod: nameof(ReaderPrefix), finalizer: nameof(ReaderFinalizer), origType: OTypes,
                    NullPatchType: true);
            if (Flag == LockFlag.WriterLock)
                RimThreadedHarmony.Prefix(Original, typeof(MethodLocker), Method, destructive: false,
                    PatchMethod: nameof(WriterPrefix), finalizer: nameof(WriterFinalizer), origType: OTypes,
                    NullPatchType: true);
        }

        /// <summary>
        ///     Locks a method on the declaring type of the original method. Use this for classes that do not have an instance if
        ///     you need to lock static methods.
        ///     These methods SupportsRecursion so be careful to not deadlock yourself.
        /// </summary>
        /// <param name="Original">The type of what you want to lock</param>
        /// <param name="Method">The name of the method you want to lock</param>
        ...
        public static void LockMethodOnDeclaringType(Type Original, string Method, LockFlag Flag, Type[] OTypes = null)
        {
            if (Flag == LockFlag.ReaderLock)
                RimThreadedHarmony.Prefix(Original, typeof(MethodLocker), Method, destructive: false,
                    PatchMethod: nameof(ReaderPrefixMB), finalizer: nameof(ReaderFinalizerMB), origType: OTypes,
                    NullPatchType: true);
            if (Flag == LockFlag.WriterLock)
                RimThreadedHarmony.Prefix(Original, typeof(MethodLocker), Method, destructive: false,
                    PatchMethod: nameof(WriterPrefixMB), finalizer: nameof(WriterFinalizerMB), origType: OTypes,
                    NullPatchType: true);
        }
        ...
        public static void LockMethodOn(Type Original, string Method, LockFlag Flag, ReaderWriterLockSlim LockOn,
            Type[] OTypes = null)
        {
            ...
            if (Flag == LockFlag.ReaderLock)
                RimThreadedHarmony.Prefix(Original, typeof(MethodLocker), Method, destructive: false,
                    PatchMethod: nameof(ReaderPrefixG), finalizer: nameof(ReaderFinalizerG), origType: OTypes,
                    NullPatchType: true);
            if (Flag == LockFlag.WriterLock)
                RimThreadedHarmony.Prefix(Original, typeof(MethodLocker), Method, destructive: false,
                    PatchMethod: nameof(WriterPrefixG), finalizer: nameof(WriterFinalizerG), origType: OTypes,
                    NullPatchType: true);
        }
        ...
        {
            if (!DeclaringTypeToLock.ContainsKey(__originalMethod.DeclaringType) ||
                DeclaringTypeToLock[__originalMethod.DeclaringType] is null)
            {
                Log.Message("RimThreaded is creating a new ReaderWriterLockSlim for DeclaringType:\t" +
                            __originalMethod.Name);
                DeclaringTypeToLock[__originalMethod.DeclaringType] =
        ...
        public static void WriterPrefix(object __instance)
        {
            ...
            {
                Log.Message("RimThreaded is creating a new ReaderWriterLockSlim for instance:\t" + __instance);
