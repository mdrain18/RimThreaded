...
namespace RimThreaded.Mod_Patches
{
    public class WorkGiver_ConstructDeliverResources_Transpile
    {
        ...
        {
            var original = typeof(WorkGiver_ConstructDeliverResources);
            var patched = typeof(WorkGiver_ConstructDeliverResources_Transpile);
            RimThreadedHarmony.Transpile(original, patched, "ResourceDeliverJobFor", null,
                new[] {"CodeOptimist.JobsOfOpportunity"});
        }

        public static IEnumerable<CodeInstruction> ResourceDeliverJobFor(IEnumerable<CodeInstruction> instructions,
            ILGenerator iLGenerator)
        {
            var list2 = iLGenerator.DeclareLocal(typeof(List<Thing>));
            var minNum = iLGenerator.DeclareLocal(typeof(int));
            //LocalBuilder resTotalAvailable = iLGenerator.DeclareLocal(typeof(int));   8
            var thingEnum = iLGenerator.DeclareLocal(typeof(IEnumerator<Thing>));
            var item = iLGenerator.DeclareLocal(typeof(Thing));
            ...
            while (i < instructionsList.Count)
            {
                ...
                if (
                    ...
                    (MethodInfo) instructionsList[i + 7].operand ==
                    AccessTools.Method(typeof(WorkGiver_ConstructDeliverResources), "FindAvailableNearbyResources")
                )
                {
                    ...
                    yield return new CodeInstruction(OpCodes.Callvirt,
                        AccessTools.Method(typeof(List<Thing>), "Clear"));
                    ...
                    yield return new CodeInstruction(OpCodes.Ldfld,
                        AccessTools.Field(
                            AccessTools.TypeByName("RimWorld.WorkGiver_ConstructDeliverResources+<>c__DisplayClass9_1"),
                            "foundRes"));
                    ...
                    yield return new CodeInstruction(OpCodes.Ldfld, AccessTools.Field(typeof(Thing), "def"));
                    yield return new CodeInstruction(OpCodes.Callvirt,
                        AccessTools.Method(typeof(Pawn_CarryTracker), "MaxStackSpaceEver"));
                    yield return new CodeInstruction(OpCodes.Call,
                        AccessTools.Method(typeof(Mathf), "Min", new[] {typeof(int), typeof(int)}));
                    ...
                    var IL_0235 = iLGenerator.DefineLabel();
                    ...
                    yield return new CodeInstruction(OpCodes.Ldfld,
                        AccessTools.Field(
                            ...
                            "foundRes"));
                    yield return new CodeInstruction(OpCodes.Callvirt,
                        AccessTools.Method(typeof(Thing), "get_Position"));
                    yield return new CodeInstruction(OpCodes.Ldloc_S, 7);
                    yield return new CodeInstruction(OpCodes.Ldfld,
                        AccessTools.Field(
                            AccessTools.TypeByName("RimWorld.WorkGiver_ConstructDeliverResources+<>c__DisplayClass9_1"),
                    ...
                    yield return new CodeInstruction(OpCodes.Ldc_I4_0);
                    yield return new CodeInstruction(OpCodes.Call,
                        AccessTools.Method(typeof(GenRadial), "RadialDistinctThingsAround"));
                    yield return new CodeInstruction(OpCodes.Callvirt,
                        AccessTools.Method(typeof(IEnumerable<Thing>), "GetEnumerator"));
                    ...
                    var IL_021E = iLGenerator.DefineLabel();
                    var tryStart = new CodeInstruction(OpCodes.Br_S, IL_021E);
                    ...
                    //IL_01d5: stloc.s 19
                    var IL_01CE = iLGenerator.DefineLabel();
                    var Ldloc_S_IL_01CE = new CodeInstruction(OpCodes.Ldloc_S, thingEnum.LocalIndex);
                    Ldloc_S_IL_01CE.labels.Add(IL_01CE);
                    yield return Ldloc_S_IL_01CE;
                    yield return new CodeInstruction(OpCodes.Callvirt,
                        AccessTools.Method(typeof(IEnumerator<Thing>), "get_Current"));
                    ...
                    var IL_01DF = iLGenerator.DefineLabel();
                    yield return new CodeInstruction(OpCodes.Ldloc_S, 8);
                    yield return new CodeInstruction(OpCodes.Ldloc_S, minNum.LocalIndex);
                    ...
                    var ldloc_s_IL_01DF = new CodeInstruction(OpCodes.Ldloc_S, item.LocalIndex);
                    ...
                    yield return new CodeInstruction(OpCodes.Call,
                        AccessTools.Method(typeof(GenAI), "CanUseItemForWork"));
                    ...
                    // end loop
                    var CI_IL_021E = new CodeInstruction(OpCodes.Ldloc_S, thingEnum.LocalIndex);
                    CI_IL_021E.labels.Add(IL_021E);
                    yield return CI_IL_021E;
                    yield return new CodeInstruction(OpCodes.Callvirt,
                        AccessTools.Method(typeof(IEnumerator), "MoveNext"));
                    ...
                    var startFinally = new CodeInstruction(OpCodes.Ldloc_S, thingEnum.LocalIndex);
                    startFinally.blocks.Add(new ExceptionBlock(ExceptionBlockType.BeginFinallyBlock));
                    yield return startFinally;
                    var IL_0234 = iLGenerator.DefineLabel();
                    ...
                    yield return new CodeInstruction(OpCodes.Callvirt,
                        AccessTools.Method(typeof(IDisposable), "Dispose"));
                ...
                //ldsfld class [mscorlib]System.Collections.Generic.List`1<class Verse.Thing> RimWorld.WorkGiver_ConstructDeliverResources::resourcesAvailable
                    ...
                    instructionsList[i].opcode == OpCodes.Ldsfld && instructionsList[i].operand
                        .Equals(AccessTools.Field(typeof(WorkGiver_ConstructDeliverResources), "resourcesAvailable"))
                )
                {
                    instructionsList[i].opcode = OpCodes.Ldloc_S;
                    instructionsList[i].operand = list2.LocalIndex;
