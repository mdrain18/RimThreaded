...
{
    internal class GiddyUpCore_Patch
    {
        ...
        public static void Patch()
        {
            ...
            var patched = typeof(GiddyUpCore_Patch);
            ...
            {
                var methodName = "DeleteExtendedDataFor";
                Log.Message("RimThreaded is patching " + giddyUpCoreStorageExtendedDataStorage.FullName + " " +
                            methodName);
            ...
            {
                var methodName = "<waitForRider>b__8_0";
        ...
        public static IEnumerable<CodeInstruction> WaitForRider(IEnumerable<CodeInstruction> instructions,
            ILGenerator iLGenerator)
        {
            ...
            {
                var currentInstruction = instructionsList[currentInstructionIndex];
                if (currentInstructionIndex >= 1)
                {
                    var lastInstruction = instructionsList[currentInstructionIndex - 1];
                    if (lastInstruction.opcode == OpCodes.Call)
                        if ((MethodInfo) lastInstruction.operand ==
                            Method(giddyUpCoreJobsJobDriver_Mounted, "get_Rider"))
        ...
        public static IEnumerable<CodeInstruction> DeleteExtendedDataFor(IEnumerable<CodeInstruction> instructions,
            ILGenerator iLGenerator)
        {
            var typeDictionaryIntData =
                typeof(Dictionary<,>).MakeGenericType(typeof(int), giddyUpCoreStorageExtendedPawnData);
            ...
            searchInstructions.Add(new CodeInstruction(OpCodes.Callvirt,
                Method(typeDictionaryIntData, "Remove", new[] {typeof(int)})));
            ...
            while (currentInstructionIndex < instructionsList.Count)
                if (IsCodeInstructionsMatching(searchInstructions, instructionsList, currentInstructionIndex))
                {
                    ...
                    foreach (var codeInstruction in GetLockCodeInstructions(
                                 ...
                                 loadLockObjectInstructions, typeDictionaryIntData))
        ...
        public static IEnumerable<CodeInstruction> GetExtendedDataFor(IEnumerable<CodeInstruction> instructions,
            ILGenerator iLGenerator)
        {
            var typeDictionaryIntData =
                typeof(Dictionary<,>).MakeGenericType(typeof(int), giddyUpCoreStorageExtendedPawnData);
            ...
            searchInstructions.Add(new CodeInstruction(OpCodes.Callvirt,
                Method(typeDictionaryIntData, "set_Item", new[] {typeof(int)})));
            //searchInstructions.Add(new CodeInstruction(OpCodes.Pop));
