...
namespace RimThreaded.RW_Patches
{
    public class ShootLeanUtility_Patch
    {
        ...
        {
            var original = typeof(ShootLeanUtility);
            var patched = typeof(ShootLeanUtility_Patch);
        ...
        private static void ReturnWorkingBlockedArray(bool[] ar)
        {
            ...
            if (conBlockedArrays.Count > 128) Log.ErrorOnce("Too many blocked arrays to be feasible. >128", 388121);
        }

        private static bool[] GetWorkingBlockedArray()
        {
            if (!conBlockedArrays.TryDequeue(out var boolArray)) boolArray = new bool[8];
            return boolArray;
        }

        public static bool LeanShootingSourcesFromTo(IntVec3 shooterLoc, IntVec3 targetPos, Map map,
            List<IntVec3> listToFill)
        {
            ...
            var angleFlat = (targetPos - shooterLoc).AngleFlat;
            var flag = angleFlat > 270f || angleFlat < 90f;
            var flag2 = angleFlat > 90f && angleFlat < 270f;
            ...
            for (var j = 0; j < 4; j++)
                if (!workingBlockedArray[j] && (j != 0 || flag) && (j != 1 || flag4) && (j != 2 || flag2) &&
                    (j != 3 || flag3) && (shooterLoc + GenAdj.AdjacentCells[j]).GetCover(map) != null)
                    lock (listToFill)
                    {
