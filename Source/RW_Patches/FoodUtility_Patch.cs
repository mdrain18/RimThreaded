...
namespace RimThreaded.RW_Patches
{
    internal class FoodUtility_Patch
    {
        ...
        {
            var original = typeof(FoodUtility);
            var patched = typeof(FoodUtility_Patch);
        ...
        {
            var ingestible = source.ingestible;
            if (ingestible == null)
            {
            ...
            if (ingestible.sourceDef != null && ingestible.sourceDef.race != null &&
                ingestible.sourceDef.race.Humanlike)
            {
                __result = MeatSourceCategory.Humanlike;
                return false;
            }

            __result = ingestible.sourceDef != null && ingestible.sourceDef.race.FleshType != null &&
                       ingestible.sourceDef.race.FleshType == FleshTypeDefOf.Insectoid
