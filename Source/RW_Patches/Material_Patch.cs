...
namespace RimThreaded.RW_Patches
{
    internal class Material_Patch
    {
        ...
        private static readonly MethodInfo methodGetTextureScaleAndOffsetImpl =
            Method(typeof(Material), "GetTextureScaleAndOffsetImpl", new[] {typeof(int)});
        ...
        private static readonly Func<object[], object> funcGetTextureScaleAndOffsetImpl2 = parameters =>
            funcGetTextureScaleAndOffsetImpl((Material) parameters[0], (int) parameters[1]);


        private static readonly MethodInfo methodHasProperty =
            Method(typeof(Material), "HasProperty", new[] {typeof(string)});
        ...
        private static readonly Func<object[], object> funcHasProperty2 =
            parameters => funcHasProperty((Material) parameters[0], (string) parameters[1]);
        ...
        private static readonly Func<object[], object> funcget_mainTexture2 =
            parameters => funcget_mainTexture((Material) parameters[0]);

        internal static void RunDestructivePatches()
        {
            var original = typeof(Material);
            var patched = typeof(Material_Patch);
            RimThreadedHarmony.harmony.Patch(Method(original, "SetFloat", new[] {typeof(string), typeof(float)}),
                new HarmonyMethod(Method(patched, nameof(SetStringFloat))));
            RimThreadedHarmony.harmony.Patch(Method(original, "SetFloat", new[] {typeof(int), typeof(float)}),
                new HarmonyMethod(Method(patched, nameof(SetIntFloat))));
            RimThreadedHarmony.harmony.Patch(Method(original, "SetInt", new[] {typeof(string), typeof(int)}),
                new HarmonyMethod(Method(patched, nameof(SetStringInt))));
            RimThreadedHarmony.harmony.Patch(Method(original, "SetInt", new[] {typeof(int), typeof(int)}),
                new HarmonyMethod(Method(patched, nameof(SetIntInt))));
            RimThreadedHarmony.harmony.Patch(Method(original, "get_mainTexture", Type.EmptyTypes),
                new HarmonyMethod(Method(patched, nameof(get_mainTexture))));
            RimThreadedHarmony.harmony.Patch(Method(original, "GetTextureScaleAndOffsetImpl", new[] {typeof(int)}),
                new HarmonyMethod(Method(patched, nameof(GetTextureScaleAndOffsetImpl))));
            RimThreadedHarmony.harmony.Patch(Method(original, "HasProperty", new[] {typeof(string)}),
                new HarmonyMethod(Method(patched, nameof(HasProperty))));
            RimThreadedHarmony.harmony.Patch(
                Method(original, "SetTextureOffsetImpl", new[] {typeof(int), typeof(Vector2)}),
                new HarmonyMethod(Method(patched, nameof(SetTextureOffsetImpl))));
        }
        ...
        public static bool GetTextureScaleAndOffsetImpl(Material __instance, ref Vector4 __result, int name)
        {
            ...
            threadInfo.safeFunctionRequest = new object[]
                {funcGetTextureScaleAndOffsetImpl2, new object[] {__instance, name}};
            mainThreadWaitHandle.Set();
            threadInfo.eventWaitStart.WaitOne();
        ...
        public static bool SetTextureOffsetImpl(Material __instance, int name, Vector2 offset)
        {
            if (!CurrentThread.IsBackground || !allWorkerThreads.TryGetValue(CurrentThread, out var threadInfo))
                return true;
            threadInfo.safeFunctionRequest = new object[]
                {ActionSetTextureOffsetImpl2, new object[] {__instance, name, offset}};
            mainThreadWaitHandle.Set();
            threadInfo.eventWaitStart.WaitOne();
            return false;
        }
        ...
        public static bool SetStringFloat(Material __instance, string name, float value)
        {
            if (!CurrentThread.IsBackground || !allWorkerThreads.TryGetValue(CurrentThread, out var threadInfo))
                return true;
            threadInfo.safeFunctionRequest = new object[]
                {ActionSetStringFloat2, new object[] {__instance, name, value}};
            mainThreadWaitHandle.Set();
            threadInfo.eventWaitStart.WaitOne();
            return false;
        }
        ...
        public static bool SetIntFloat(Material __instance, int nameID, float value)
        {
            if (!CurrentThread.IsBackground || !allWorkerThreads.TryGetValue(CurrentThread, out var threadInfo))
                return true;
            threadInfo.safeFunctionRequest = new object[]
                {ActionSetIntFloat2, new object[] {__instance, nameID, value}};
            mainThreadWaitHandle.Set();
            threadInfo.eventWaitStart.WaitOne();
            return false;
        }
