...
namespace RimThreaded.RW_Patches
{
    public class FloodFiller_Patch
    {
        ...
        {
            var original = typeof(FloodFiller);
            var patched = typeof(FloodFiller_Patch);
            RimThreadedHarmony.Prefix(original, patched, "FloodFill",
                new[]
                {
                    typeof(IntVec3), typeof(Predicate<IntVec3>), typeof(Func<IntVec3, int, bool>), typeof(int),
            ...
            IEnumerable<IntVec3> extraRoots = null)
        {
            lock (__instance)
            {
                ...
                else
                {
                    ...
                    var directionsAround = GenAdj.CardinalDirectionsAround;
                    var length = directionsAround.Length;
                    var cellIndices = __instance.map.cellIndices;
                    var num1 = 0;
                    __instance.openSet.Clear();
                    if (root.IsValid)
                    {
                        var index = cellIndices.CellToIndex(root);
                    ...
                    if (extraRoots != null)
                    {
                        if (extraRoots is IList<IntVec3> intVec3List)
                            for (var index1 = 0; index1 < intVec3List.Count; ++index1)
                            {
                                var index2 = cellIndices.CellToIndex(intVec3List[index1]);
                            ...
                            {
                                var index = cellIndices.CellToIndex(extraRoot);
                                __instance.visited.Add(index);
                                __instance.traversalDistance[index] = 0;
                    ...
                    if (rememberParents)
                        for (var index = 0; index < __instance.visited.Count; ++index)
                        {
                            var cell = cellIndices.IndexToCell(__instance.visited[index]);
                            __instance.parentGrid[__instance.visited[index]] = passCheck(cell) ? cell : IntVec3.Invalid;
                        }

                    while (__instance.openSet.Count > 0)
                    {
                        var c1 = __instance.openSet.Dequeue();
                        var num2 = __instance.traversalDistance[cellIndices.CellToIndex(c1)];
                        if (!processor(c1, num2))
                        {
                            ...
                            if (num1 != maxCellsToProcess)
                            {
                                ...
                                {
                                    var c2 = c1 + directionsAround[index1];
                                    var index2 = cellIndices.CellToIndex(c2);
                                ...
                                Log.Error("Overflow on flood fill (>" + area +
                                          " cells). Make sure we're not flooding over the same area after we check it.");
                                __instance.working = false;
                                return false;
                            }
                        }
