...
namespace RimThreaded.RW_Patches
{
    public class Pawn_Patch
    {
        ...
        {
            var original = typeof(Pawn);
            var patched = typeof(Pawn_Patch);
            RimThreadedHarmony.Prefix(original, patched,
                nameof(Destroy)); //causes strange crash to desktop without error log
        ...
        public static void ThingWithCompsPostApplyDamage(ThingWithComps __instance, DamageInfo dinfo,
        ...
        {
            var mindState = __instance.mindState;
            __result = !__instance.Dead && mindState.mentalStateHandler.InMentalState &&
                       mindState.mentalStateHandler.CurStateDef.IsAggro;
            return false;
        }
        ...
        public static bool Destroy(Pawn __instance, DestroyMode mode = DestroyMode.Vanish)
        {
            ...
            {
                var assignedGrave = __instance.ownership.AssignedGrave;
                __instance.ownership.UnclaimAll();
                if (mode == DestroyMode.KillFinalize) assignedGrave?.CompAssignableToPawn.TryAssignPawn(__instance);
            }

            __instance.ClearMind(false, true);
            var lord = __instance.GetLord();
            if (lord != null)
            {
                var cond = mode != DestroyMode.KillFinalize ? PawnLostCondition.Vanished : PawnLostCondition.Killed;
            ...
            if (Pawn_PlayerSettings_Patch.petsInit == false) Pawn_PlayerSettings_Patch.RebuildPetsDictionary();
            if (Pawn_PlayerSettings_Patch.pets.TryGetValue(__instance, out var pawnList))
            ...
            */
            if (__instance.equipment != null) __instance.equipment.Notify_PawnDied();

            if (mode != DestroyMode.KillFinalize)
            {
                if (__instance.equipment != null) __instance.equipment.DestroyAllEquipment();

                __instance.inventory.DestroyAll();
                if (__instance.apparel != null) __instance.apparel.DestroyAll();
            }
        ...
        public static void ThingDestroy(Thing __instance, DestroyMode mode = DestroyMode.Vanish)
        {
            ...
            var spawned = __instance.Spawned;
            var map = __instance.Map;
            if (__instance.Spawned) __instance.DeSpawn(mode);

            __instance.mapIndexOrState = -2;
            if (__instance.def.DiscardOnDestroyed) __instance.Discard();

            var compExplosive = __instance.TryGetComp<CompExplosive>();
            if (spawned)
            {
                var list = compExplosive != null ? new List<Thing>() : null;
                GenLeaving.DoLeavingsFor(__instance, map, mode, list);
                compExplosive?.AddThingsIgnoredByExplosion(list);
            }

            if (__instance.holdingOwner != null) __instance.holdingOwner.Notify_ContainedItemDestroyed(__instance);

            RemoveAllReservationsAndDesignationsOnThis(__instance);
            if (!(__instance is Pawn)) __instance.stackCount = 0;
        ...
        {
            if (__instance.def.category == ThingCategory.Mote) return;
            ...
            for (var i = 0; i < maps.Count; i++)
            {
                ...
                var attackTarget = __instance as IAttackTarget;
                if (attackTarget != null) maps[i].attackTargetReservationManager.ReleaseAllForTarget(attackTarget);
        ...
        {
            if (__instance.jobs == null || __instance.CurJob != null || __instance.jobs.jobQueue.Count > 0 ||
                __instance.jobs.startingNewJob) return false;
            var flag = false;
            var maps = Find.Maps;
            for (var i = 0; i < maps.Count; i++)
            {
                var obj = maps[i].reservationManager.FirstReservationFor(__instance);
                if (obj.IsValid)
                {
                    Log.ErrorOnce(
                        $"Reservation manager failed to clean up properly; {__instance.ToStringSafe()} still reserving {obj.ToStringSafe()}",
                        0x5D3DFA5 ^ __instance.thingIDNumber);
                    flag = true;
                }

                var obj2 = maps[i].physicalInteractionReservationManager.FirstReservationFor(__instance);
                if (obj2.IsValid)
                {
                    Log.ErrorOnce(
                        $"Physical interaction reservation manager failed to clean up properly; {__instance.ToStringSafe()} still reserving {obj2.ToStringSafe()}",
                        0x12ADECD ^ __instance.thingIDNumber);
                    flag = true;
                }

                var attackTarget = maps[i].attackTargetReservationManager.FirstReservationFor(__instance);
                if (attackTarget != null)
                {
                    Log.ErrorOnce(
                        $"Attack target reservation manager failed to clean up properly; {__instance.ToStringSafe()} still reserving {attackTarget.ToStringSafe()}",
                        0x5FD7206 ^ __instance.thingIDNumber);
                    flag = true;
                }

                var obj3 = maps[i].pawnDestinationReservationManager.FirstObsoleteReservationFor(__instance);
                if (obj3.IsValid)
                {
                    var job = maps[i].pawnDestinationReservationManager.FirstObsoleteReservationJobFor(__instance);
                    Log.Warning(
                        $"Pawn destination reservation manager failed to clean up properly; {__instance.ToStringSafe()}/{job.ToStringSafe()}/{job.def.ToStringSafe()} still reserving {obj3.ToStringSafe()}");
                    flag = true;
                }
            }
