...
namespace RimThreaded.RW_Patches
{
    public class Region_Patch
    {
        ...
        {
            var original = typeof(Region);
            var patched = typeof(Region_Patch);
        ...
        {
            var cells = GetRegionCells(__instance);
            if (cells.Count == 0)
            {
        ...
        {
            if (!RegionCells.TryGetValue(region, out var regionCells))
            {
                regionCells = new HashSet<IntVec3>();
                var regions = region.Map.regionGrid;
                for (var z = region.extentsClose.minZ; z <= region.extentsClose.maxZ; z++)
                for (var x = region.extentsClose.minX; x <= region.extentsClose.maxX; x++)
                {
                    var intVec = new IntVec3(x, 0, z);
                    if (regions.GetRegionAt_NoRebuild_InvalidAllowed(intVec) == region) regionCells.Add(intVec);
                }
        ...
        */
        ...
        {
            if (regionClosedIndex.TryGetValue(region, out var closedIndex)) return closedIndex;
        ...
        {
            var regionTraverserReplacements = new Dictionary<OpCode, MethodInfo>();
            ...
            RimThreadedHarmony.replaceFields.Add(Field(typeof(Region), nameof(Region.closedIndex)),
                regionTraverserReplacements);
        }
        ...
        public static bool DangerFor(Region __instance, ref Danger __result, Pawn p)
        {
            if (Current.ProgramState == ProgramState.Playing)
            {
                ...
                {
                    var localCachedDangers = __instance.cachedDangers;
                    for (var index = 0; index < localCachedDangers.Count; ++index)
            ...
            var danger = Danger.Unspecified;
            var room = __instance.Room;
            District district = null;
            if (room != null) district = __instance.District;
            if (room != null && district != null)
            {
                ...
                if (Current.ProgramState == ProgramState.Playing)
                {
                    ...
                    if (!Region.cachedSafeTemperatureRanges.TryGetValue(p, out floatRange))
                        lock (cachedSafeTemperatureRange)
                        {
                            if (!Region.cachedSafeTemperatureRanges.TryGetValue(p, out var floatRange2))
                            {
                ...
                danger = !floatRange.Includes(temperature)
                    ? !floatRange.ExpandedBy(80f).Includes(temperature) ? Danger.Deadly : Danger.Some
                    : Danger.None;
                if (Current.ProgramState == ProgramState.Playing)
