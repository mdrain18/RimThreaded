...
namespace RimThreaded.RW_Patches
{
    public class ThingOwnerThing_Patch
    {
        ...
        {
            var original = typeof(ThingOwner<Thing>);
            var patched = typeof(ThingOwnerThing_Patch);
            RimThreadedHarmony.TranspileLockAdd3(original, "TryAdd", new[] {typeof(Thing), typeof(bool)});
            ...
            while (i < instructionsList.Count)
            {
                if (i + 9 < instructionsList.Count && instructionsList[i + 9].opcode == OpCodes.Callvirt)
                    if (instructionsList[i + 9].operand is MethodInfo methodInfo)
                        if (methodInfo.Name.Contains("RemoveAt") &&
                            methodInfo.DeclaringType.FullName.Contains("System.Collections"))
                        {
                            ...
                            foreach (var lockInstruction in RimThreadedHarmony.EnterLock(lockObject, lockTaken,
                                         loadLockObjectInstructions, instructionsList[i])) yield return lockInstruction;
                            var collectionType = methodInfo.DeclaringType;
                            var collectionCopy = iLGenerator.DeclareLocal(collectionType);
                            var constructorInfo = collectionType.GetConstructor(new[] {collectionType});
                            var storeReplay = new List<CodeInstruction>();
                            ...
                            var j = 0;
                            while (j < storeReplay.Count - 1) yield return storeReplay[j++];
                            yield return new CodeInstruction(OpCodes.Ldloc, collectionCopy.LocalIndex);
                            yield return new CodeInstruction(OpCodes.Stfld, storeReplay[j].operand);

                            foreach (var lockInstruction in RimThreadedHarmony.ExitLock(iLGenerator, lockObject,
                                         lockTaken, instructionsList[i])) yield return lockInstruction;
                            continue;
                        }
